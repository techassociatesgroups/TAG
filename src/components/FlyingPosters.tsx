...
layout(location = 2) in vec2 aModelUvs;
...
void main() {
    ...
    float radius = length(worldPosition.xyz);
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale }) => {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      try {
        ...
        console.error('Failed to initialize FlyingPosters WebGL:', error);
      }
    }
  }, [items, scale, windowWidth, isInView]);
  ...
  return (
    <div ref={containerRef} className="w-full h-full bg-black relative min-h-[500px] flex items-center justify-center">
      {!sketch && (
      ...
      <canvas 
        ref={canvasRef} 
        className={`w-full h-full transition-opacity duration-700 ${sketch ? 'opacity-100' : 'opacity-0'}`} 
