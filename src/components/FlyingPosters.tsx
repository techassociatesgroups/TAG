...
class InfiniteGridMenu {
  ...
  private animate(deltaTime: number): void {
    ...
    if (!this.control.isPointerDown && this.control.rotationVelocity < 0.01) {
      const autoRotateAngle = deltaTime * 0.00004; // Control rotation speed
      const autoRotation = quat.setAxisAngle(quat.create(), [0, 1, 0], autoRotateAngle); // Rotate around Y axis
      quat.multiply(this.control.orientation, autoRotation, this.control.orientation);
    }
  ...
  private updateProjectionMatrix(): void {
    ...
    if (this.camera.aspect > 1) {
      ...
      // zoom out more on narrow (mobile) viewports so discs don't dominate
      this.camera.fov = 2 * Math.atan(height / (this.camera.aspect * 0.6) / distance);
    }
    mat4.perspective(
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale }) => {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      try {
        console.log('Initializing FlyingPosters with canvas:', canvas);
        console.log('Canvas dimensions:', canvas.clientWidth, canvas.clientHeight);
        const computedScale = (() => {
          if (typeof scale === 'number') return scale;
          if (windowWidth < 480) return 0.6;
          if (windowWidth < 768) return 0.8;
          if (windowWidth < 1024) return 0.95;
          return 1.0;
        })();

        const newSketch = new InfiniteGridMenu(
          canvas,
          items.length ? items : defaultItems,
          sk => {
            console.log('FlyingPosters initialized, starting render loop');
            sk.run();
          },
          computedScale
        );
        ...
        console.error('Failed to initialize FlyingPosters:', error);
      }
    } else {
      console.warn('Canvas ref is null');
    }
  }, [items, scale, windowWidth]);
  ...
  const wrapperStyle: React.CSSProperties = {
    width: '100%',
    // on mobile: use specific height; on desktop: fill parent
    height: windowWidth < 640 ? '420px' : '100%',
    backgroundColor: '#000'
  };

  return (
    <div className="relative w-full h-full overflow-hidden" style={wrapperStyle}>
      <canvas
        id="infinite-grid-menu-canvas"
        ref={canvasRef}
        className="cursor-grab absolute inset-0 outline-none active:cursor-grabbing z-0"
